...
import 'package:rive/rive.dart'; //masoct animation with rive
...
class ERPLoopPage extends StatefulWidget { //stateful widget because the animations change 
...
class _ERPLoopPageState extends State<ERPLoopPage> with TickerProviderStateMixin{
  ...
  late AudioPlayer _audioPlayer; //for both web and android (plays from url and filepath)
  ...
  void initState() {
    ...
    @override
  void dispose() {
    _pieAnimationController.dispose(); // Dispose the controller when the widget is removed
    super.dispose();
  }
  ...
  _startTimer();
    ...
    _audioPlayer.play();
    await Future.delayed(Duration(seconds: 1)); // delaying a bit before the animation is played
    _startTalking();
    ...
    _stopTalking();
    freeze= true;
    await Future.delayed(Duration(seconds: 1)); //delaying a second after each loop
  }
  setState(() {
  ...
  void _startRecording() async {
    ...
    if (!kIsWeb) {
      var status = await Permission.microphone.request(); //for android permisssion to use the mic is asked
      if (!status.isGranted) {
        return;
      }
      final directory = await getApplicationDocumentsDirectory(); //accessing the directory 
      final path = '${directory.path}/myFile.m4a'; //geting a path to store the recording (m4a format)
      await record.start(const RecordConfig(), path: path);
    }
    else{
       _microphoneRecorder.start(); //for web the package takes care of asking for permission automatically
    }

    Future.delayed(Duration(milliseconds: 500), () { // a little delay to make sure the animation completes
      setState(() {
  ...
  if (recordingPath != null && File(recordingPath!).existsSync()) { //making sure the recording is set to the path in android
...
hasRecording = true;}
  ...
  void _handleTap() { //chaning the rive animation and functions based on state and gesture
      ...
      children: [
        ...
        Row(
          ...
          children: [
            ...
            // Looping Button
            ElevatedButton(
              onPressed: freeze || isTalking || !hasRecording ? null : _playRecording, // disable button under these conditions
